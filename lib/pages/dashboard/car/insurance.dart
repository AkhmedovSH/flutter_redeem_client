...
class InsuranceState extends State<Insurance> {
  ...
  getInsurance() async {
    final response = await get('/services/mobile/api/insurance/${Get.arguments['id']}');
    setState(() {
      sendData = response;
  ...
  create() async {
    final response = await post('/services/mobile/api/insurance', sendData);
    if (response != null) {
      if (response['success']) {
        Get.back(result: 1);
      }
  ...
  update() async {
    final response = await put('/services/mobile/api/insurance', sendData);
    if (response != null) {
      if (response['success']) {
        Get.back(result: 1);
      }

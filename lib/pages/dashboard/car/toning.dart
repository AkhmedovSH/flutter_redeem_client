...
class Toning extends StatefulWidget {
  const Toning({Key? key}) : super(key: key);

  @override
  State<Toning> createState() => ToningState();
}

class ToningState extends State<Toning> {
  ...
  getToning() async {
    final response = await get('/services/mobile/api/toning/${Get.arguments['id']}');
    setState(() {
      sendData = response;
      data['beginDate'].text = response['beginDate'];
      data['endDate'].text = response['endDate'].toString();
    });
  }

  create() async {
    final response = await post('/services/mobile/api/toning', sendData);
    if (response != null) {
      if (response['success']) {
        Get.back(result: 1);
      }
  ...
  update() async {
    final response = await put('/services/mobile/api/toning', sendData);
    if (response != null) {
      if (response['success']) {
        Get.back(result: 1);
      }
